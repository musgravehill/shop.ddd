...
final class OrderComment extends ValueObjectAbstract implements ValueObjectInterface
{
    //self-validation
    public function __construct(
        private readonly string $comment
    ) {
        if (!preg_match('/^[\w\d\s\+\-\?\$\.\*\(\)\[\],~!@#№%&=:;<>_\/"]{0,512}$/u', $comment)) {
            throw new InvalidArgumentException('Rule: comment should be simple text 0..512.');
        }
    }
    ...
    {
        return sha1(serialize([$this->comment])); //you can add params (color white-red, size S-M-L) to array
    }
