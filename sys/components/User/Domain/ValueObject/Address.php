...
final class Address extends ValueObjectAbstract implements ValueObjectInterface
{
    //self-validation
    public function __construct(
        private readonly string $address
    ) {
        if (!preg_match('/^[\w\d\s\+\-\?\$\.\*\(\)\[\],~!@#№%&=:;<>_\/"]{0,512}$/u', $address)) {
            throw new InvalidArgumentException('Rule: address should be simple text 0..512.');
        }
    }

    public function getStructuralEqualityIdentifier(): string
    {
        return sha1(serialize([$this->address])); //you can add params (color white-red, size S-M-L) to array
    }
