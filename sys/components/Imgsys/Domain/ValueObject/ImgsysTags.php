...
final class ImgsysTags extends ValueObjectAbstract implements ValueObjectInterface
{
    //self-validation
    public function __construct(
        private readonly string $tags
    ) {
        if (mb_strlen($tags, "utf-8") > 512) {
            throw new InvalidArgumentException('Rule: tags should be simple text 0..512.');
        }
        if (!preg_match('/^[\w\d\s]{0,512}$/u', $tags)) {
            throw new InvalidArgumentException('Rule: tags should be simple text 0..512.');
        }
    }
    ...
    {
        return sha1(serialize([$this->tags])); //you can add params (color white-red, size S-M-L) to array
    }
